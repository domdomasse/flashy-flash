...
const TABS = [
  ...
  { id: 'summary', label: 'Résumé', short: 'Résumé', iconName: 'file-text' },
  { id: 'flashcards', label: 'Flashcards', short: 'Flash.', iconName: 'layers' },
  { id: 'glossary', label: 'Glossaire', short: 'Gloss.', iconName: 'book-text' },
  { id: 'exercises', label: 'Exercices', short: 'Exos', iconName: 'pencil' }
];
...
export async function renderChapter(container, { subject: subjectId, chapter: chapterId, tab }) {
  ...
  for (const t of TABS) {
    const btn = el('button', {
      ...
      icon(t.iconName, 18),
      el('span', { class: 'tab-bar-label' }, t.label),
      el('span', { class: 'tab-bar-label-short' }, t.short)
    );
  ...
  const header = el('div', { class: 'chapter-header' }, topbar, tabBar);
